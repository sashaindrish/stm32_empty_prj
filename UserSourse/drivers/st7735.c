...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////ST7735_RST_PORT_CLK |
  ...
  RCC_APB2PeriphClockCmd(ST7735_A0_PORT_CLK |  ST7735_CS_PORT_CLK , ENABLE);
...
void ST7735_PutChar7x11_b(uint16_t X, uint16_t Y, uint8_t chr, uint16_t color, uint16_t bgcolor)
{
  uint16_t i,j;
  uint8_t buffer[11];
  uint8_t CH = color >> 8;
  ...
  {
    // ASCII[0x20-0x7F]
    memcpy(buffer,&Font7x11_b[(chr - 32) * 11], 11);////////////////////////////////////////////////////////////////////////////////////////////
  }
  else if (chr >= 0xA0)
  {
    // CP1251[0xA0-0xFF]
    memcpy(buffer,&Font7x11_b[(chr - 64) * 11], 11); ///////////////////////////////////////////////////////////////////////////////////////////////
  }
  else
  {
    // unsupported symbol
    memcpy(buffer,&Font7x11_b[160], 11); /////////////////////////////////////////////////////////////////////////////////////////////////////////////
  }
...
void ST7735_PutStr7x11_b(uint8_t X, uint8_t Y, char *str, uint16_t color, uint16_t bgcolor)
{
  while (*str)
  {
    ST7735_PutChar7x11_b(X,Y,*str++,color,bgcolor);
    if (X < scr_width - 8) { X += 8; } else if (Y < scr_height - 12) { X = 0; Y += 12; } else { X = 0; Y = 0; }
  };
}
