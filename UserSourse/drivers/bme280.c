...
void BME280_WriteReg(uint8_t reg, uint8_t value) {
	...
	I2C_StartTransmission(I2C1, I2C_Direction_Transmitter,  BME280_ADDR);
...
uint8_t BME280_ReadReg(uint8_t reg) {
	...
	I2C_StartTransmission(I2C1, I2C_Direction_Receiver,  BME280_ADDR);
		value=I2C_ReadData(I2C1);
		I2C_StopTransmission(I2C1);
...
BME280_RESULT BME280_Read_Calibration(void) {
	...
	I2C_Mem_Read(I2C1, BME280_ADDR,buf[0],(uint8_t *)&cal_param, 25);
...
//	if (!I2Cx_Write(BME280_I2C_PORT,&buf[0],1,BME280_ADDR,I2C_NOSTOP)) return BME280_ERROR;
	...
	if (I2C_Mem_Read(I2C1,  BME280_ADDR,BME280_REG_PRESS_MSB, &buf[0], 2)) {///I2Cx_Read(BME280_I2C_PORT,&buf[0],3,BME280_ADDR)
...
BME280_RESULT BME280_Read_UT(int32_t *UT) {
	...
	*UT = 0x80000;
	...
	// Read the 'temp' register (_msb, _lsb, _xlsb)I2Cx_Read(BME280_I2C_PORT,&buf[0],3,BME280_ADDR)
	if (I2C_Mem_Read(I2C1,  BME280_ADDR,BME280_REG_TEMP_MSB, &buf[0], 2)) {
...
BME280_RESULT BME280_Read_UH(int32_t *UH) {
	...
	*UH = 0x8000;
	...
	// Read the 'hum' register (_msb, _lsb)I2Cx_Read(BME280_I2C_PORT,&buf[0],2,BME280_ADDR
	if (I2C_Mem_Read(I2C1,  BME280_ADDR,BME280_REG_TEMP_MSB, &buf[0], 1)) {
...
BME280_RESULT BME280_Read_UTPH(int32_t *UT, int32_t *UP, int32_t *UH) {
	...
	*UH = 0x8000;
	...
	// Read the 'press', 'temp' and 'hum' registersI2Cx_Read(BME280_I2C_PORT,&buf[0],8,BME280_ADDR)
	if (I2C_Mem_Read(I2C1,  BME280_ADDR,BME280_REG_PRESS_MSB, &buf[0], 7)) {
		*UP = (int32_t)((buf[0] << 12) | (buf[1] << 4) | (buf[2] >> 4));
...
uint8_t BME280_get_results(float *hu,float *pr, float *tm,float *pr_mrg){
	...
	*tm = BME280_CalcTf(UT);
	*pr = BME280_CalcPf(UP);
	*pr_mrg = BME280_Pa_to_mmHg(BME280_CalcP(UP));
...
void BME280_init(void){
	...
	BME280_SetStandby(BME280_STBY_1s);//BME280_STBY_1s
